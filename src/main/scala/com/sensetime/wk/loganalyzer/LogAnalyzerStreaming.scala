package com.sensetime.wk.loganalyzer

import com.sensetime.wk.model.ApacheAccessLog
...
object LogAnalyzerStreaming extends App {
  ...
  val accessLogsDStream: DStream[ApacheAccessLog] = logLinesDStream.map(ApacheAccessLog.parseLogLine).filter(_!=null).cache()
